...
def _popen_kwargs() -> dict[str, int]:
    """Return OS-level flags that keep test subprocesses off the user's
    visible session.

    Windows: ``CREATE_NO_WINDOW`` so no console flashes when the
    parent process has no console of its own.
    Unix: ``start_new_session=True`` which maps to ``setsid(2)`` —
    the same detach strategy used by ``shell.rs`` and ``sandbox.rs``.
    """
    kwargs: dict[str, int] = {}
    if os.name == "nt":
        # ``CREATE_NO_WINDOW`` is defined on ``subprocess``; in some
        # narrow Python builds it is missing, in which case we
        # hard-code the documented value (0x08000000).
        flags = getattr(subprocess, "CREATE_NO_WINDOW", 0x08000000)
        kwargs["creationflags"] = flags
    else:
        # ``start_new_session`` is the documented asyncio-friendly
        # equivalent of ``setsid``; it is always present on Python
        # 3.9+ and we require >= 3.11 at install time.
        kwargs["start_new_session"] = 1
    return kwargs
