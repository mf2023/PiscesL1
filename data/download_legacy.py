...
# Ensure MODELSCOPE_CACHE_DIR is defined and points to project-root/.pisceslx/datatmp
...
try:
    ...
    # Provide clearer diagnostics about environment versions
    try:
...
def detect_available_splits(dataset_name):
    """
    ...
    for split in split_candidates:
        try:
            if MODELSCOPE_AVAILABLE:
                MsDataset.load(dataset_name, split=split)
            else:
    ...
    for attempt in range(max_retries):
        try:
            ...
            load_methods = [
                ...
                {'kwargs': {}, 'desc': 'direct load'},
                # Method 2: Try different splits
                {'kwargs': {'split': 'train'}, 'desc': 'with split=train'},
                {'kwargs': {'split': 'validation'}, 'desc': 'with split=validation'},
                {'kwargs': {'split': 'test'}, 'desc': 'with split=test'},
                # Method 3: Try using the default split
                {'kwargs': {'split': 'default'}, 'desc': 'with split=default'}
            ]
