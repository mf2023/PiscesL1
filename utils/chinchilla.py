...
def scale_to_existing_block(parameters: float, existing_blocks: list = None) -> Tuple[int, int]:
    ...
    if existing_blocks is None:
        # Default Arctic configurations (layers, hidden_size)
        existing_blocks = [
            (22, 896),   # ~0.5B
            (24, 2048),  # ~1.5B  
            (32, 2560),  # ~7B
    ...
    min_diff = abs(self_estimate_params(*best_config) - parameters)
    
    for config in existing_blocks[1:]:
        params = self_estimate_params(*config)
...
def self_estimate_params(layers: int, hidden_size: int, 
    ...
    """
    Estimate model parameters for Arctic architecture.
