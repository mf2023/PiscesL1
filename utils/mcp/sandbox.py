...
"""
Tool safety sandbox for Ruchbah agentic system.

This module provides secure tool execution with:
...
logger = PiscesLxCoreLog("pisceslx.tools.agentic.sandbox")


@dataclass
class PiscesL1SandboxConfig:
...
class PiscesL1ExecutionResult:
    ...
    def __repr__(self) -> str:
        ...
        return f"PiscesL1ExecutionResult({status}, time={self.execution_time:.3f}s)"


class PiscesL1ToolSandbox:
    """Secure tool execution sandbox.
    
    Provides safe execution of untrusted tool code with:
    - Resource limits (memory, CPU, time)
    ...
    def __init__(self, config: Optional[PiscesL1SandboxConfig] = None):
        """Initialize tool sandbox.
        
        Args:
            config: Sandbox configuration.
        """
        self.config = config or PiscesL1SandboxConfig()
        ...
        logger.info("PiscesL1ToolSandbox initialized")
        logger.info(f"  - Timeout: {self.config.timeout_seconds}s")
        logger.info(f"  - Memory limit: {self.config.max_memory_mb}MB")
        logger.info(f"  - Network allowed: {self.config.allow_network}")
        logger.info(f"  - Isolation level: {self.config.isolation_level}")
        ...
        **kwargs
        ...
        try:
            ...
            try:
                ...
                return PiscesL1ExecutionResult(
                    success=(exit_code == 0),
                    output=output,
    ...
    def _run_in_process(
        ...
        **kwargs
        ...
        try:
            ...
            result = PiscesL1ExecutionResult(
                success=False,
    ...
    def _execute_code(
        ...
        **kwargs
        ...
        """Execute code with proper language handler."""
        handlers = {
    ...
    def _get_restricted_builtins(self) -> Dict[str, Any]:
        """Get restricted builtins for safe execution."""
        safe_builtins = {
            "print": print,
            "len": len,
            "str": str,
    ...
    def _run_unsafe(
        ...
        **kwargs
        ...
        """Run code without sandbox (for debugging)."""
        logger.warning("Running code WITHOUT sandbox - FOR DEBUGGING ONLY")
        
        return self._execute_code(code, language, **kwargs)
    
    def _get_container_command(self, code: str, language: str) -> str:
        """Get command to run in container."""
        import base64
...
def create_tool_sandbox(
    config: Optional[PiscesL1SandboxConfig] = None,
) -> PiscesL1ToolSandbox:
    """Factory function to create tool sandbox.
    
    Args:
        config: Sandbox configuration.
        
    Returns:
        Initialized tool sandbox.
    """
    return PiscesL1ToolSandbox(config)


if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description="Tool sandbox for PiscesL1")
    ...
    config = PiscesL1SandboxConfig(
        timeout_seconds=args.timeout,
        max_memory_mb=args.memory,
        allow_network=args.network,
    )
    
    sandbox = create_tool_sandbox(config)
    
    if args.test:
        with sandbox.run("test_tool", "print('Hello, Sandbox!')", "python") as result:
            print(f"Result: {result}")
            print(f"Output: {result.output}")
    
    print(sandbox.get_stats())
    
    logger.success("Sandbox test completed")
