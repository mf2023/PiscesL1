    ...
    def __init__(
        ...
        if self.enabled:
            try:
                ...
                _LOG.warning("inference_watermark_operator_unavailable", error=str(e))
                self._content_wm = None
...
@PiscesLxOperatorRegistrar()
class InferencePipelineWatermarkOperator(PiscesLxTransformOperator):
    def __init__(self, config: Optional[PiscesLxOperatorConfig] = None, enabled: bool = True):
        ...
        if config is not None:
            ...
            enabled = bool(params.get("enabled", enabled))
        self.enabled = enabled
        self._integrator = InferenceWatermarkIntegrationOperator(
