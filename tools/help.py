...
def help():
    ...
    print("")
    print("  Distillation:")
    print("    --distill                     Enable knowledge distillation training")
    print("    --teacher_model <path/url>    Teacher model: local path, server URL, or API base URL")
    print("    --teacher_name <str>          Model name for API: gpt-4, llama3.1, etc. (for remote)")
    print("    --teacher_api_key <str>       API key for remote API authentication")
    print("    --teacher_type <str>          Teacher source: local, server, remote (default: local)")
    print("    --distill_alpha <float>       Distillation loss weight 0.0-1.0 (default: 0.5)")
    print("    --distill_temperature <float> Temperature for soft labels (default: 2.0)")
    print("")
    ...
    print("  dev         Manage developer mode for training (vim-style command interface)")
    print("    enable                       Enable developer mode")
    print("    disable                      Disable developer mode")
    print("    status                       Show developer mode status")
    print("")
    print("  Available Commands (when dev mode enabled during training):")
    print("  Memory:")
    print("    :mem [module]                Show memory details")
    print("    :mem-gpu                     Show GPU memory allocation")
    print("    :mem-cpu                     Show CPU memory usage")
    print("")
    print("  Model:")
    print("    :layer <n>                   Show layer information")
    print("    :layers                      List all layers with parameters")
    print("    :grad                        Show gradient statistics")
    print("    :grad-norm                   Show gradient norm history")
    print("")
    print("  Training Control:")
    print("    :pause                       Pause training")
    print("    :resume                      Resume training")
    print("    :save [name]                 Save checkpoint")
    print("    :lr [value]                  Get/set learning rate")
    print("    :batch [size]                Get/set batch size")
    print("")
    print("  Configuration:")
    print("    :config                      Show training config")
    print("    :config-model                Show model config")
    print("    :config-data                 Show data config")
    print("")
    print("  Monitoring:")
    print("    :watch <var>                 Watch a variable")
    print("    :watch-clear                 Clear watch list")
    print("    :profile [type]              Profile system (gpu/cpu/memory)")
    print("")
    print("  Intervention:")
    print("    :inject <path> [op]          Inject into model parameters")
    print("      Operations: zeros, ones, freeze, unfreeze, <number>,")
    print("                  normal(mean,std), uniform(a,b), xavier, kaiming,")
    print("                  scale(f), clip(min,max), add(v), negate, abs,")
    print("                  sqrt, square, log, exp, sin, cos, tan,")
    print("                  relu, sigmoid, tanh, copy <src_path>")
    print("    :freeze <layer>              Freeze layer parameters")
    print("    :unfreeze <layer>            Unfreeze layer parameters")
    print("    :nan-check                   Check for NaN/Inf values")
    print("")
    print("  Other:")
    print("    :help                        Show all commands")
    print("    :q                           Close overlay")
    print("")
    print("Cache:")
    print("  cache       Manage .pisceslx cache directory")
    print("    status                       Show cache status (default)")
    print("    clean                        Clean all cache (settings/ protected)")
    print("")
    print("Examples:")
    print("  # Core")
    ...
    print("")
    print("  # Distillation")
    print("  python manage.py train --model_size 0.5B --dataset Chinese2 --distill --teacher_model /path/to/teacher.pt")
    print("  python manage.py train --model_size 7B --dataset Chinese2 --distill --teacher_model http://localhost:8000 --teacher_type server")
    print("  python manage.py train --model_size 7B --dataset Chinese2 --distill --teacher_model http://localhost:11434/v1 --teacher_name llama3.1 --teacher_type remote")
    print("  python manage.py train --model_size 7B --dataset Chinese2 --distill --teacher_model https://api.openai.com/v1 --teacher_name gpt-4 --teacher_api_key sk-xxxxx --teacher_type remote")
    print("  python manage.py train --model_size 7B --dataset Chinese2 --distill --teacher_model /path/to/teacher.pt --distill_alpha 0.7 --distill_temperature 3.0")
    print("")
    ...
    print("")
    print("  # Publish (Package models as Docker images with inference engine)")
    print("  python manage.py publish --publish_action full --publish_model_size 7B --publish_registry docker.io")
    print("  python manage.py publish --publish_action full --publish_model_size 7B --publish_model_path ./ckpt/7B.pt")
    print("  python manage.py publish --publish_action export --publish_model_size 7B --publish_output_dir ./export/")
    print("  python manage.py publish --publish_action build --publish_model_size 7B --publish_template gpu")
    print("  python manage.py publish --publish_action push --publish_registry ghcr.io --publish_registry_namespace myuser")
    print("  python manage.py publish --publish_action validate --publish_model_size 7B")
    print("  python manage.py publish --publish_action info --publish_model_size 7B")
    print("  python manage.py publish --publish_action list")
    print("  python manage.py publish --publish_action full --publish_model_size 7B --publish_export_quantize --publish_quant_bits 4")
    print("  python manage.py publish --publish_action full --publish_model_size 7B --publish_metadata_name 'PiscesLX-7B' --publish_metadata_version '1.0.0'")
    print("")
    print("Publish:")
    print("  publish    Package and publish models as Docker images")
    print("    --publish_action <str>        Action: full, export, build, push, validate, info, list (default: full)")
    print("    --publish_model_size <str>    Model size: 0.5B, 1B, 7B, 14B, 72B, 671B, 1T")
    print("    --publish_model_path <path>   Path to model checkpoint for publishing")
    print("    --publish_output_dir <path>    Output directory for publish artifacts")
    print("    --publish_registry <str>      Container registry URL (default: docker.io)")
    print("    --publish_registry_namespace <str>  Registry namespace (user or organization)")
    print("    --publish_registry_repo <str>  Repository name for the image")
    print("    --publish_registry_tag <str>  Image tag (default: latest)")
    print("    --publish_username <str>       Username for registry authentication")
    print("    --publish_password <str>       Password or token for registry authentication")
    print("    --publish_template <str>       Docker template: default, minimal, gpu (default: default)")
    print("    --publish_export_format <str>  Export format: safetensors, pytorch (default: safetensors)")
    print("    --publish_export_quantize      Export model with quantization")
    print("    --publish_quant_bits <int>     Quantization bits: 2, 4, 8 (default: 4)")
    print("    --publish_base_image <str>     Base Docker image")
    print("    --publish_env_vars <str>       Environment variables (KEY1=VALUE1,KEY2=VALUE2)")
    print("    --publish_port <int>           Inference server port inside Docker (default: 8000)")
    print("    --publish_health_check         Enable health check (default: True)")
    print("    --publish_no_health_check      Disable health check")
    print("    --publish_retry_count <int>   Retry count for registry push (default: 3)")
    print("    --publish_retry_delay <int>   Retry delay seconds (default: 5)")
    print("    --publish_metadata_name <str>  Model name for metadata")
    print("    --publish_metadata_version <str>  Model version (default: 1.0.0)")
    print("    --publish_metadata_description <str>  Model description")
    print("    --publish_metadata_author <str>  Model author")
    print("    --publish_metadata_license <str>  Model license (default: Apache-2.0)")
    print("    --publish_generate_checksum    Generate checksum files (default: True)")
    print("    --publish_no_checksum          Skip checksum generation")
    print("    --publish_checksum_algorithms <str>  Checksum algorithms (default: sha256,md5)")
    print("    --publish_config <path>         Publish config file (JSON/YAML)")
    print("")
    print("  # Developer Mode")
    print("  python manage.py dev enable")
    print("  python manage.py dev disable")
    print("  python manage.py dev status")
    print("")
    print("  # Cache")
    print("  python manage.py cache")
    print("  python manage.py cache status")
    print("  python manage.py cache clean")
