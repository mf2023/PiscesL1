...
[Documentation](https://dunimd.github.io/enta) | [Changelog](docs/CHANGELOG.md) | [Security](docs/SECURITY.md) | [Contributing](docs/CONTRIBUTING.md) | [Code of Conduct](docs/CODE_OF_CONDUCT.md)
...
EnTA uses a multi-language architecture with three major components, letting each layer use the most appropriate language for its domain:

<div align="center">

| Language | Component | Purpose |
|:---------|:----------|:--------|
| **Python** | `backend/enta/` | AI Agent framework core — agent loop, 31 LLM backends, 36 built-in tools, 18 adapters, safety engine, memory, skills, swarm, hooks, LSP, MCP |
| **Rust** | `native/crates/enta-core` | Native high-performance library — file I/O, regex search, glob, diff, sandbox, tokenizer, BM25 indexer, Landlock, SIMD search, LSP protocol, semantic embeddings |
| **Rust** | `native/crates/enta-py` | PyO3 bindings exposing the Rust core to Python as `enta._native` |
| **TypeScript** | `desktop/` | EnTA Desktop — Electron-based desktop app with React UI, Monaco Editor, xterm.js terminal, IPC bridge to the Python backend |

</div>

EnTA is shipped as a single repository with three components that share the same release cycle. There is no standalone library or separately installable package — clone the whole repository to use EnTA.

### Repository Structure

```
d:\enta/
├── pyproject.toml             # Python build & dependencies (builds backend/enta/)
...
│   │   ├── native.py          #   Python wrapper over enta._native
...
│   └── crates/
│       ├── enta-core/        #   Native core (package name: `enta`)
│       │   ├── Cargo.toml
│       │   └── src/
│       │       ├── lib.rs
...
│       └── enta-py/          # PyO3 bindings → `enta._native`
│           ├── Cargo.toml
│           └── src/lib.rs
│
...
- **Catalog**: MCP Catalog, native model catalog (`enta.backends.catalog`)
...
- Each sub-agent is defined as a `SubAgentConfig` and loaded from `enta/agents/builtin.py` with prompt templates under `enta/prompts/skills/`.
...
- **External protocol**: MCP client (`enta.tools.mcp` + `MCPManager`) for Model Context Protocol servers
...
- Each adapter implements `BaseAdapter` and is registered through `enta.adapters.manager`.
...
- **Bash command analyzer** with dangerous-pattern detection (`enta.safety.analyze_bash_command`, `dangerous_commands.txt`)
- **SSRF guard** combining DNS resolution with CIDR blocklists (`enta.ssrf.EncreSSRFGuard`)
- **Docker container sandbox** (`enta.sandbox.container.EncreContainerSandbox`) with seccomp-style isolation
- **Rust-level sandbox** (`enta-core/sandbox.rs`) with Linux **Landlock** enforcement (`landlock.rs`)
- **Auto safety classifier** (`enta.autosafety.EncreAutoSafetyClassifier`) — AI-based permission decisions
- **Rate limiter** (`enta.ratelimit.EncreRateLimiter`) for backends and tool calls

#### 💾 Persistent Memory (`enta.memdir`)
...
- **Dynamic system-prompt generation** via `enta.prompts.system.EncrePromptBuilder`
- Skill generator + consolidator (`enta.learning`) for runtime skill creation
...
- **Task system**: typed CRUD with bash / agent / workflow executors (`enta.task`)

#### 🧱 Native Rust Core (`native/crates/enta-core`)
...
- **Context compaction** (`enta.compact`) — 9 strategies: Always, Auto, TokenBudget, BudgetReduction, Semantic, Snip, MicroCompact, ContextCollapse, MultiStagePipeline
...
- **Codebase indexer** — BM25 search + dependency graph (`enta.codebase`)
...
- **CLI runner** — `python -m enta.iclaw` (iClaw mode)
- **Gateway protocol** — `GatewayServer` / `GatewayClient`

<h2 align="center">🔧 Development Setup</h2>

### Prerequisites
- **Python**: 3.11+ (the `enta` package requires `>=3.11`)
...
pip install -e ".[native]"          # Pre-built native extension (enta-native)
...
```bash
# One-shot: builds the Rust extension, copies _native into backend/enta/,
...
# The compiled _native.pyd / _native.so is then copied into backend/enta/
...
```bash
# iClaw is a long-running automation runner (enta/iclaw/__main__.py)
...
EnTA Desktop is an Electron + React 19 AI chat application:
...
compact:
  strategy: "auto"               # one of the 9 enta.compact strategies
...
1. Built-in defaults (`enta.config.EncreConfig`)
2. Configuration files (YAML, TOML)
3. Environment variables (prefixed with `ENTA_`)
4. Programmatic configuration passed to `EncreAgent(...)`

<h2 align="center">❓ Frequently Asked Questions</h2>

**Q: How many LLM backends does EnTA support?**
A: **31** backends: OpenAI, Anthropic, DeepSeek, Google Gemini, Groq, Ollama, Local (HuggingFace), Alibaba (Qwen / DashScope), Tencent, Xiaomi, Kimi, GLM, MiniMax, Arcee, Novita, OpenRouter, AWS Bedrock, GitHub Copilot, LM Studio, OpenAI Compatible, OpenAI SSE, AI Gateway, Kilocode, OpenCode, plus the meta-backends Failover, Router, Retry, and the MCP Catalog and the static `enta.backends.catalog` provider catalog.

**Q: How many built-in tools ship with EnTA?**
A: **36** built-in tools under `enta/tools/builtin/`, plus a fully pluggable tool registry and an MCP client for external tools.

**Q: How do I add a custom tool?**
A: Subclass `EncreTool` (defined in `enta/tools/base.py`), implement `execute(**kwargs) -> str` and the formatting methods, then register the tool with the `ToolRegistry` (or use the discovery mechanism in `enta/tools/discovery.py`).

**Q: What are the permission / safety modes?**
A: **6** modes: `bypass` (no checks), `dont_ask` (auto-allow), `accept_edits` (auto-allow edits), `plan` (plan first), `auto` (heuristic), `default` (ask for confirmation). They are surfaced through `enta.utils.types.PermissionMode`.

**Q: Is this a PyPI library I can `pip install`?**
A: No. EnTA is shipped as a single repository containing Python, Rust, and Electron/TypeScript code; the three components share a single release cycle. To use it, clone the repository and follow the Development Setup guide. The Python package can then be installed from source via `pip install -e .`.

**Q: How does the memory system work?**
A: Memory is stored as frontmatter-parsed `.md` files in a `memdir/` directory with a `MEMORY.md` index (`enta.memdir`). The system tracks freshness/age, automatically prunes aged entries, and exposes semantic search backed by NumPy similarity.

**Q: Does EnTA support multi-agent workflows?**
A: Yes. The **Swarm** system (`enta.swarm`) provides teammates, a concurrency-limited swarm manager, an async mailbox, an orchestrator, a blackboard, and a consensus protocol. The **Task** system (`enta.task`) provides typed CRUD with bash / agent / workflow executors. There are also **9 built-in sub-agents** defined in `enta/agents/builtin.py` (coder, researcher, critic, architect, planner, spec-writer, monitor, executor, scheduler).

**Q: How does EnTA use Rust?**
A: Performance-critical operations — file I/O, regex search, glob, unified diff, SIMD pattern matching, BM25 indexing, sandbox / Landlock, token counting, semantic similarity, and the LSP JSON-RPC parser — are implemented in Rust (`native/crates/enta-core`) and exposed to Python through PyO3 (`enta._native`). The extension is built with `python build.py` or `cargo build --release -p enta-py`.
...
A: Under `enta/adapters/` — 18 adapters implementing `BaseAdapter`, including Discord, Slack, Telegram, DingTalk, Feishu, WeCom, Weixin, WhatsApp, Signal, SMS, Email, Matrix, MS Graph, Home Assistant, QQ Bot, BlueBubbles, Yuanbao, and a generic Webhook adapter.

**Q: How do I run EnTA as a server?**
A: `enta.server` exposes a WebSocket server and an admin HTTP API. Use `EncreServer` / `run_server` from `enta.server.app` (lazy-loaded from `enta/__init__.py`) and the `SessionManager` from `enta.server.session_manager`.
